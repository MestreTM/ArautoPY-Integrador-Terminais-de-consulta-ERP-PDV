(function () {
  ...
  /* ---- confirmação de limpeza (mesmo estilo do progresso) ---- */
  function confirmarLimpezaEnvio() {
    return new Promise((resolve) => {
      const box = $("prop-confirm-envio");
      if (!box) {
      ...
      const ok = $("prop-confirm-envio-ok");
      const cancel = $("prop-confirm-envio-cancel");
      const fechar = (val) => {
        box.hidden = true;
      ...
      if (ok) ok.onclick = () => fechar(true);
      if (cancel) cancel.onclick = () => fechar(false);
      box.onclick = (e) => { if (e.target === box) fechar(false); };
      box.hidden = false;
    });
  }
  ...
  $("prop-salvar-config") && ($("prop-salvar-config").onclick = async () => {
    if (!(await confirmarLimpezaEnvio())) return;
